...
func WriteJSONWithStatus(w http.ResponseWriter, data interface{}, status int) {
	...
	log.WithFields(logrus.Fields{"response": string(d), "status": status}).Info("http json response")
...
// JSONResponse wraps JSON responses (including error) similar to above
// methods but with additional log-fields specified by caller.
type JSONResponse struct {
	logger *logrus.Entry
	rw     http.ResponseWriter
}

// NewJSONResponse is the factory method of JSONResponse
func NewJSONResponse(w http.ResponseWriter, logFields logrus.Fields) *JSONResponse {
	return &JSONResponse{
		rw:     w,
...
// Write is similar to WriteJSON but with a method receiver.
func (s *JSONResponse) Write(data interface{}) {
	s.WriteWithStatus(data, http.StatusOK)
}

// WriteErrorWithStatus is similar to WriteJSONErrorWithStatus but with a method receiver.
func (s *JSONResponse) WriteErrorWithStatus(err error, status int) {
	d, err := json.Marshal(map[string]string{"error": err.Error()})
	if err != nil {
		s.writeWithStatus([]byte("marshal json error"), http.StatusInternalServerError)
		return
	}
	s.writeWithStatus(d, status)
}

// WriteWithStatus is similar to WriteJSONWithStatus but with a method receiver.
func (s *JSONResponse) WriteWithStatus(data interface{}, status int) {
	d, err := DataToJSON(data)
	if err != nil {
		s.writeWithStatus(ErrorToJSON(err), http.StatusInternalServerError)
		return
	}
	s.writeWithStatus(d, status)
}

// writeWithStatus is similar to writeWithStatus but with a method receiver.
func (s *JSONResponse) writeWithStatus(data []byte, status int) {
	s.rw.Header().Set("Content-Type", "application/json")
	s.rw.WriteHeader(status)
	_, err := s.rw.Write(data)
	if err != nil {
		s.logger.WithField("error", err).Error("failed to write response")
	} else {
		s.logger.WithFields(logrus.Fields{"response": string(data), "status": status}).Info("http json response")
