...
GITBRANCH=$(shell git rev-parse --abbrev-ref HEAD | grep -oE "^([A-Z]+-[0-9]+|master)")
...
ifeq ($(GITBRANCH),master)
	BUILDTAG=$(VERSION)-$(GITHASH)
	NOMAD_NAME=$(REPO_NAME)
else
	BUILDTAG=$(VERSION)-$(GITHASH)-$(GITBRANCH)
...
.PHONY: build clean deploy help test test-v docker docker-build docker-artifacts docker-push docker-test docker-clean docker-run docker-run-stop bamboo-test bamboo-publish bamboo-clean rename rename-revert rename-clean
...
rename: rename-bamboo-specs rename-conf-files rename-readme
...
rename-revert: rename-revert-bamboo-specs rename-revert-conf-files
...
rename-clean: rename-clean-bamboo-specs rename-clean-conf-files
...
help:
	...
	@echo " $ make deploy	-- creates the docker image and deploys it to docker-registry.electric.net and pushes binary to artifacts server."
